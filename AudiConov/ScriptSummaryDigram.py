...
mmdc_path = "mmdc.cmd"  # or, for example: r"C:\Users\gurin\AppData\Roaming\npm\mmdc.cmd"
...
def expand_code_editor():
    # Get current sash coordinates (sash index 0)
    x, y = paned_window.sash_coord(0)
    # Move the sash to the right (increasing code editor width)
    paned_window.sash_place(0, x + 20, y)

def expand_diagram_viewer():
    x, y = paned_window.sash_coord(0)
    # Move the sash to the left (increasing diagram viewer width)
    paned_window.sash_place(0, x - 20, y)
...
def on_mousewheel(event):
    # For Windows: event.delta is multiple of 120; on Linux, use Button-4 and Button-5 events instead
