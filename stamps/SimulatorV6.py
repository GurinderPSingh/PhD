...
###############################################################################
# LINEAR ELEMENT STAMPING FUNCTIONS
###############################################################################
...
def vcvs_stamp_custom(G, C, L, RHS, nop, nom, nip, nim, gain):
    ...
    idx = size_old
    if nop > 0:
        G_expanded[nop-1, idx] += 1; G_expanded[idx, nop-1] += 1
    if nom > 0:
        G_expanded[nom-1, idx] -= 1; G_expanded[idx, nom-1] -= 1
    if nip > 0:
...
def stamp_nonlinear_devices(G, RHS, x, elements, models, node_offset):
    for elem in elements:
        if elem["type"] == "D":
            ...
            if n_p is not None: G[n_p, n_p] += Gd
            if n_m is not None: G[n_m, n_m] += Gd
            if n_p is not None and n_m is not None:
                G[n_p, n_m] -= Gd; G[n_m, n_p] -= Gd
            IncI = Id - Gd*Vd
            if n_p is not None: RHS[n_p] -= IncI
            if n_m is not None: RHS[n_m] += IncI
        elif elem["type"] == "BJT":
...
def stamp_nonlinear_bjt(G, RHS, x, elem, models, node_offset):
    ...
    if nb is not None: RHS[nb] -= mismatch_BE
    if ne is not None: RHS[ne] += mismatch_BE
    # Controlled collector current I_C = alpha_F * I_BE
    I_C = alpha_F * I_BE
    G_C = alpha_F * G_BE
    if nc is not None: RHS[nc] -= I_C
    if ne is not None: RHS[ne] += I_C
...
def stamp_nonlinear_mos(G, RHS, x, elem, models, node_offset):
    # Simplified n-channel MOSFET using square-law model
    nd = elem["nd"] - 1
    ng = elem["ng"] - 1
    ns = elem["ns"] - 1
    model = models.get(elem["model"], {})
    ...
    if V_GS <= Vth:
        ...
        G_m = k * (V_GS - Vth)
    if nd is not None: RHS[nd] -= I_D
    if ns is not None: RHS[ns] += I_D
    if ng is not None:
...
###############################################################################
# TRANSIENT ANALYSIS FUNCTION (using dense direct solver by default)
###############################################################################
...
if __name__ == "__main__":
    ...
    print("4) Iterative Solver Analysis (DC system)")
    ...
    if analysis_choice == "1":
        ...
        # AC Analysis
        print("\n--- Running AC Analysis ---")
        use_iterative = input("Is your AC circuit complex? (yes/no): ").strip().lower()
        ...
        if use_iterative.startswith("y"):
            ...
            # Use iterative solver for each frequency point
            ...
            methods = {
                ...
                "4": iterative_minres,
                "5": iterative_jacobi,
                "6": lambda A,b,**kwargs: iterative_sor(A, b, omega=1.5, **kwargs),
                "7": iterative_multigrid,
                "8": iterative_ilu_gmres,
                "9": lambda A,b,**kwargs: (sparse_direct_solver(A, b), 0, 0)
            }
            solver = methods.get(iter_choice, None)
            if solver is None:
                print("Invalid choice; using direct solver.")
            for i, f in enumerate(tqdm(freqs, desc="Frequency Sweep")):
                omega = 2 * np.pi * f
                s = 1j * omega
                M = G + s * C + s * L
                if solver:
        ...
        with PdfPages("ac_plots.pdf") as pdfp:
            plt.figure()
            for i in range(sum(1 for n in unknowns if n.startswith("V"))):
                plt.semilogx(freqs, np.abs(x_solutions[:, i]), label=unknowns[i])
            plt.xlabel("Frequency [Hz]"); plt.ylabel("Magnitude"); plt.title("AC Magnitude")
            plt.legend(); plt.grid(True, which="both")
            pdfp.savefig(); plt.close()
            plt.figure()
            for i in range(sum(1 for n in unknowns if n.startswith("V"))):
                plt.semilogx(freqs, np.angle(x_solutions[:, i], deg=True), label=unknowns[i])
        ...
        t_points, x_transient, unknowns_trans = run_transient_analysis(elements, tmax=tmax, dt=dt)
        if use_iterative.startswith("y"):
            ...
            transient_methods = {
                ...
                "3": iterative_jacobi,
                "4": lambda A, b, tol=1e-9, maxiter=1000: iterative_sor(A, b, tol=tol, maxiter=maxiter, omega=1.5),
                "5": iterative_ilu_gmres,
                "6": lambda A, b, tol=1e-9, maxiter=1000: (sparse_direct_solver(A, b), 0, 0)
            }
            solver_trans = transient_methods.get(iter_choice, None)
            if solver_trans is None:
                print("Invalid choice; using direct solver.")
            new_dim = x_transient.shape[1]
            x_transient_iter = np.zeros((len(t_points), new_dim))
            G_t = np.zeros((num_nodes, num_nodes), dtype=float)
            C_t = np.zeros((num_nodes, num_nodes), dtype=float)
            RHS_t = np.zeros((num_nodes), dtype=float)
            ...
            stability_note = "Stability issues detected during transient iterative analysis." if not stable else "No stability issues detected."
        ...
        for i in range(sum(1 for n in unknowns_trans if n.startswith("V"))):
            plt.plot(t_points, x_transient[:, i], label=unknowns_trans[i])
        ...
        with PdfPages(pdf_filename) as pdf_out:
            fig_analysis = make_text_page(analysis_text, f"DC Iterative Analysis Method {iter_choice}")
