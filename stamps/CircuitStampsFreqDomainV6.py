...
if __name__ == "__main__":
    ...
    # Your data is naturally node-centric (e.g., voltages for each node across all frequencies).
    # This is intuitive for you or matches your circuit simulation logic.
    # frequencies = np.logspace(1, 6, 100000)  # 10 Hz to 1 MHz, 100,000 points
    # solutions = [np.random.random(len(frequencies)) + 1j * np.random.random(len(frequencies)) for _ in range(4)]  # 4 nodes
    # unknowns = [f"V_{i+1}" for i in range(len(solutions))]  # Node names

    # if Your data is frequency-centric (e.g., voltages for all nodes at each frequency point).
    # This aligns better with your simulation workflow or tools. use the following two statements
    with PdfPages("Magnitude_Response_Plots.pdf") as pdf_pages:
        ...
        for i, name in enumerate(unknowns):
            ...
            if solutions[0] is not None:
                ...
                # the following statement is used to plot the magnitude of the voltage in Logarithmic frequency axis
                plt.semilogx(frequencies, magnitudes, label=f"Magnitude of {name}")
                # if you do not want to to plot the magnitude in Logarithmic frequency axis, you can use the following statement and comment the above statement
                # plt.plot(frequencies, magnitudes, label=f"Magnitude of {name}")
                plt.title(f"Frequency Response of {name}")
                plt.xlabel("Frequency (Hz) [Log scale]")  # Logarithmic scale for frequency if log scale is not used, you can use "Frequency (Hz)" instead of "Frequency (Hz) [Log scale]"
                plt.ylabel("Voltage Magnitude")
                plt.grid(which='both', linestyle='--', linewidth=0.5)
                plt.legend()
                plt.show()
                # pdf_pages.savefig()  # Save magnitude plot to PDF
                # plt.close()

                # Plot phase
                plt.figure(figsize=(10,6))
                # the following statement is used to plot the phase of the voltage in Logarithmic frequency axis
                plt.semilogx(frequencies, phases, label=f"Phase of {name}")
                # if you do not want to to plot the phase in Logarithmic frequency axis, you can use the following statement and comment the above statement
                # plt.plot(frequencies, phases, label=f"Phase of {name}")
                ...
                plt.legend()
                plt.show()
                # pdf_pages.savefig()  # Save phase plot to PDF
                # plt.close()

                # print("All plots have been saved to 'Frequency_Response_Plots.pdf'")

                # Adjust Plot Range: If results vary significantly across frequencies, consider using logarithmic scales (plt.semilogx).
                # plt.semilogx(frequencies, magnitudes, label=f"Magnitude of {name}")
                # if required, Introduce logging or save intermediate results for long-running simulations to prevent data loss.
